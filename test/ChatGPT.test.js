...
describe( "OpenAIApi", () => {
    describe( "createCompletion()", () => {
        it( "should return the correct response when prompt is given", async () => {
            ...
            expect( response ).to.equal( "JavaScript chatbot: You can use the setTimeout() method. setTimeout(() => alert('Hello'), 10000);" );
        });
        it( "should return an error message when no api key is provided", async () => {
            ...
            expect( response ).to.equal( "No API Key provided" );
        });
    });
});
